# Photonics Lab Calculators

Small, self-contained HTML tools for common free-space optics calculations.
No build step, no dependencies beyond a Google Fonts link — each tool is a
single `index.html` file.

## Tools
- **[Experimental Setup Building tool](./Experimental-Setup-Building-tool/)** - These tool can help to choose the components like lens and objective as per requirment during experimental setup building
  - ***[Beam Coupling Calculator](./Experimental-Setup-Building-tool/beam-coupling/)*** — focused spot
    size, effective NA, and mode-match quality for free-space-to-fiber coupling
    and 4f pinhole spatial filtering (paraxial Gaussian beam optics).
  - ***[Spectrometer Signal Coupling](./Experimental-Setup-Building-tool/Spectrometer-Signal-Coupling/)*** — back-aperture
    beam diameter from your objective, and the coupling lens focal length that matches your
    spectrometer's f-number, so the grating is neither over- nor underfilled.
    
- **[Photon Energy Calculator](./photon-energy-calculator/)** — converts
  between wavelength, frequency, photon energy (eV / J), and wavenumber.

## Live pages

- `https://brijeshgangwar93.github.io/Tools/beam-coupling-calculator/`
- `https://brijeshgangwar93.github.io/Tools/photon-energy-calculator/`
- `https://brijeshgangwar93.github.io/Tools/Experimental-Setup-Building-tool/Spectrometer-Signal-Coupling/`
